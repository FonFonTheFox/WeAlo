...
useEffect(() => {
    ...
    const fetchGroupInfo = async () => {
      try {
        const response = await fetch(`http://localhost:8000/group/${_id}`);
  ...
  const fetchMessages = async () => {
    try {
      ...
      const data = await response.json();
      console.log("fetch mess group:", data);
  ...
  useEffect(() => {
    const fetchRecepientData = async () => {
      try {
        const response = await fetch(
          `http://localhost:8000/group-messages/${_id}`
        );
  ...
  const handleSend = async (messageType, imageUri) => {
    try {
      ...
      formData.append("groupId", _id); // Add _id to formData
      ...
      <ScrollView ref={scrollViewRef} contentContainerStyle={{flexGrow:1}} onContentSizeChange={handleContentSizeChange}>
        {messages.map((item, index) => {
          ...
          if (item.messageType === "image") {
            const baseUrl = "/Users/hoang/Downloads/\WeAlo/api/files/";
